...
export function PostForm({ categories, initial = {} }: { categories: { id: string; name: string }[]; initial?: Initial }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      status: submitter?.value || initial.status || "DRAFT",
    };
  ...
  return (
    <form onSubmit={submit} style={{ display: "grid", gap: 20 }}>
      ...
      <div className="card" style={{ padding: 24 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>{images.map(url => <div key={url} style={{ position: "relative" }}><Image src={url} alt="Vista previa" width={130} height={90} style={{ objectFit: "cover", borderRadius: 8 }} /><button type="button" onClick={() => setImages(x => x.filter(i => i !== url))} style={{ position: "absolute", right: 4, top: 4 }}>×</button></div>)}</div>
      </div>
